    ...
    >
      <View className="bg-red-100 p-3 rounded-full">
        {item.icon === "person" || item.icon === "people-circle" ? (
          <Ionicons name={item.icon as any} size={24} color="#F75A41" />
        ) : (
          <MaterialIcons name={item.icon as any} size={24} color="#F75A41" />
        )}
      </View>
      ...
      <View className="bg-red-50 px-3 py-1 rounded-full">
        <Text className="text-red-600 font-outfit-medium">
          {item.people}
        </Text>
      </View>
    </TouchableOpacity>
